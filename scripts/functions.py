...
class Dataset_LeTac(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        x = (x, tuple(pv_pair))
        y = torch.FloatTensor([self.labels[index]]).to(device)            
        return x, y
...
class MPClayer(nn.Module):
    ...
    def forward(self, x, gripper_p, gripper_v):
        ...
        row_single = zeros_hstack_help(I, self.nStep, self.nHidden+2, self.nHidden+2)
        AN_ = row_single
        for i in range(self.nStep-1):
            AN = I
            row_single = I
            ...
            row_single = zeros_hstack_help(row_single, self.nStep-i-1, self.nHidden+2, self.nHidden+2)
