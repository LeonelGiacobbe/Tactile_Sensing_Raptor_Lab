...
with open('./position_log.csv', 'r') as f:
    reader = csv.reader(f)
    for row in reader:
...
plt.yticks(np.arange(y_min, y_max + 0.1, 5))  # 0.1 to include top edge

# X-axis: keep all points, set ticks every 10
x_min = min(timestamps)
x_max = max(timestamps)
plt.xlim(x_min, x_max)
plt.xticks(np.arange(x_min, x_max + 0.1, 10))  # tick interval only
