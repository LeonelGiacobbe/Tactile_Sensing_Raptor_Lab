...
class Camera:
    ...
    def connect1(self):
        ...
        if self.cam is None or not self.cam.isOpened():
            print("Warning: unable to open video source %d" % (0))
        else:
            print("Connect to %s at video source %d" % (self.dev_type, 0))
    ...
    def connect2(self):
        ...
        self.cam.set(cv2.CAP_PROP_BUFFERSIZE, 1)
        if self.cam is None or not self.cam.isOpened():
            print("Warning: unable to open video source %d" % (2))
        else:
            print("Connect to %s at video source %d" % (self.dev_type, 2))
...
class FastCamera:
    ...
    def __init__(self, dev_id, imgh, imgw, raw_imgh, raw_imgw, framerate, verbose=True):
        """
