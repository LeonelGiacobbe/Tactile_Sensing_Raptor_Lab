...
def getOrientation(pts, img):
    ...
    # label = "  Rotation Angle: " + str(deg_angle - 90) + " degrees"
    # textbox = cv2.rectangle(img, (cntr[0], cntr[1]-25), (cntr[0] + 250, cntr[1] + 10), (255,255,255), -1)
    # cv2.putText(img, label, (cntr[0], cntr[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1, cv2.LINE_AA)
    
    return deg_angle
 
def find_gripper_width(minor_axis_length):
...
def process_image(frame):
    ...
    return roi_frame, binary_image

def find_ellipses(roi_frame, binary_image):
    angles = []
    centers = []
    minor_axis_lengths = []
    ...
    for i, c in enumerate(contours):
        ...
        if area < 1000:
            ...
            minor_axis_lengths.append(minEllipse[i][1][0])  # used to calculate the ideal gripper opening
            print("Minor axis length: ", minor_axis_lengths[-1])
    ...
    return centers, angles, minor_axis_lengths
