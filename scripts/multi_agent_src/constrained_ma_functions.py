...
class MPClayer(nn.Module):
    ...
    def _apply_constraints(self):
        nConstraints = 6 * self.nStep  # 3 constraints for position, velocity, and acceleration per step
        G_list = []
        h_list = []

        # For position constraints (min and max) for both agents
        for t in range(self.nStep):
            # Min position constraint for both agents (position >= min_pos)
            G_list.append(torch.cat([torch.zeros(self.nStep, self.nStep), -torch.eye(self.nStep), torch.zeros(self.nStep, self.nStep)], dim=1))
            h_list.append(torch.full((self.nStep, 1), self.min_pos))

            # Max position constraint for both agents (position <= max_pos)
            G_list.append(torch.cat([torch.zeros(self.nStep, self.nStep), torch.eye(self.nStep), torch.zeros(self.nStep, self.nStep)], dim=1))
            h_list.append(torch.full((self.nStep, 1), self.max_pos))

        # For velocity constraints (min and max) for both agents
        for t in range(self.nStep):
            # Min velocity constraint (velocity >= min_vel)
            G_list.append(torch.cat([torch.zeros(self.nStep, self.nStep), -torch.eye(self.nStep), torch.zeros(self.nStep, self.nStep)], dim=1))
            h_list.append(torch.full((self.nStep, 1), self.min_vel))

            # Max velocity constraint (velocity <= max_vel)
            G_list.append(torch.cat([torch.zeros(self.nStep, self.nStep), torch.eye(self.nStep), torch.zeros(self.nStep, self.nStep)], dim=1))
            h_list.append(torch.full((self.nStep, 1), self.max_vel))

        # For acceleration constraints (min and max) for both agents
        for t in range(self.nStep):
            # Min acceleration constraint (acceleration >= min_acc)
            G_list.append(torch.cat([torch.zeros(self.nStep, self.nStep), -torch.eye(self.nStep), torch.zeros(self.nStep, self.nStep)], dim=1))
            h_list.append(torch.full((self.nStep, 1), self.min_acc))

            # Max acceleration constraint (acceleration <= max_acc)
            G_list.append(torch.cat([torch.zeros(self.nStep, self.nStep), torch.eye(self.nStep), torch.zeros(self.nStep, self.nStep)], dim=1))
            h_list.append(torch.full((self.nStep, 1), self.max_acc))

        # Stack the constraints into G and h
        self.G = torch.vstack(G_list)
        self.h = torch.vstack(h_list)
    ...
    def forward(self, x1, x2, own_gripper_p, own_gripper_v, other_gripper_p, other_gripper_v):
        ...
        h = self.h.squeeze(-1).repeat(nBatch, 1).contiguous()
