...
class MPClayer(nn.Module):
    def __init__(self, nHidden=20, eps=1e-4, nStep=20, del_t=1/60):
        ...
        self.Kd = 1.0 # Probably wont use this as author said marker
                      # Displacement can be let as zero
        ...
        # Add constraint bounds for gripper motion
        self.pmin = 0.0  # Minimum position
        self.pmax = 140.0   # Maximum position
        self.vmin = -10.0   # Minimum velocity
        self.vmax = 10.0    # Maximum velocity
        self.amin = -30.0   # Minimum acceleration
        self.amax = 30.0    # Maximum acceleration

        # Implementing constraints after training
        self.G = Variable(torch.zeros(self.nStep, self.nStep).to(device))
        self.h = self.create_constraint_bounds()

    def create_constraint_bounds(self):
        """
            Ensure gripper input and output are dynamically feasible
        """
        # This needs to be [self.nStep, 1]
        return Variable(torch.ones(self.nStep, 1).to(device) * self.pmax)
