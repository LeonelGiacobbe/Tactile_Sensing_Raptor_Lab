...
class MPClayer(nn.Module):
    ...
    def forward(self, x, gripper_p, gripper_v):
        ...
        x_extended = torch.hstack((x, gripper_state))
        x_extended = x_extended.reshape([nBatch, 1, self.nHidden+2])
        p_x0_batch = torch.bmm(x_extended, p_batch).reshape(nBatch, self.nStep)

        # Add the linear learned parameter q_f to p_x0_batch
        q_f_expanded = self.q_f.unsqueeze(0).expand(nBatch, self.nStep, 1)
        p_qf = 2 * q_f_expanded.reshape(nBatch, self.nStep)
        p_x0_batch = p_x0_batch + p_qf
        ...
        x_predict = torch.bmm(S_batch, u.reshape(nBatch, self.nStep, 1)) + torch.bmm(T_batch, x_extended.reshape(nBatch, self.nHidden+2, 1))
...
"""
here:
    fk is the hidden feature vector (size nHidden).
    vk is the scalar velocity.
    ak is the scalar control input (acceleration, which is u_k in the QP).
    Qf is the weight matrix for features (nHidden x nHidden).
    Qv is the scalar weight for velocity squared.
    Qa is the scalar weight for control input squared.
    P is the terminal weight multiplier.
    N is the prediction horizon (nStep).

I have self.Qv ready for me.
Qf in the notes seems to be self.Af (weight matrix for Fn)
Fn in the notes seems to be u in the code
V could be gripper_v

qfTF = self.Af.t() * u
QvV = self.Qv * gripper_v

Dr. Sun also wants me to add q0 final? terminal cost?
