...
"""
    Conversion to mutli-agent mpc:
    -   combine state (x_state) and control (u) vectors
    -   Change Ad and Bd to block diagonal matrices
        - Ad_joint = block_diag(Ad_1, Ad_2)
    - Maybe make a global cost function?
        - Combine q_, r_ and qn_ matrices using block diagonal
    - Set up a single OSQP problem
