...
class ModelBasedMPCNode(Node):
    def __init__(self):
        ...
        _ = self.mpc_layer(_, dummy_mpc_var, dummy_mpc_var)          

        # Load weights
        package_dir = get_package_share_directory('tactile_sensing')
        model_path = os.path.join(package_dir, 'models', 'single_letac_mpc_model.pth')
        ...
        while rclpy.ok():
            try:
                ...
                with self.stream, torch.no_grad():
                    # Unpack data
                    (image_tensor_1, 
                     gripper_posi_1, gripper_vel_1) = data
                    ...
                    pos_sequences_1 = self.mpc_layer(
                        tactile_embeddings_1, 
                    ...
                    with self.results_lock:
                        self.latest_results = (pos_sequences_1)
    ...
    def run(self):
        try:
            ...
            with self.contact_area_lock:
                if torch.all(self.current_image_1 == 0).item():
                    return
            ...
            with self.results_lock:
                if self.latest_results is not None:
                    pos_sequences_1 = self.latest_results
